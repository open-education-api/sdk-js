...
Surfnet.OnderwijsData = function(url, token, version) {
    this.config = {endpoint: url, auth: {token: token}, version: version};

    this.init = function() {
        if (this.config.endpoint.slice(-1) != "/") {
            this.config.endpoint += "/";
        }
        // Add the version API to the url.
        if (this.config.version === undefined) {
            this.config.version = "1.0";
        }
        this.config.endpoint += this.config.version + "/";
    };
